...
class ProcessTransaction constructor(private val data: TransactionData) : DataInterface {

    override fun insertData(transaccion: Transaccion) =
        Observable.create<Int> {
            val day = Calendar.getInstance().get(Calendar.DAY_OF_WEEK)
            if (validarPlaca(transaccion.placa, day)) {
                if (getData(transaccion.placa) == null) {
    ...
    override fun updateData(transaccion: Transaccion,price:Int): Observable<Unit> =
    ...
    override fun getNumberOfTransactionsByType(type: Int): Observable<Boolean> =
        Observable.fromCallable {
